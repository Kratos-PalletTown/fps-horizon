...
public class KratosSimulation
{
    ...
    private static final int MS_SAMPLES = 10;

    public KratosSimulation() {
        this.fpsSamples = new int[15];
        this.msSamples  = new long[MS_SAMPLES];
    ...
    public void onClientTick(final TickEvent.ClientTickEvent event) {
        ...
        if (lastTickTime > 0) {
            ...
            msIndex = (msIndex + 1) % MS_SAMPLES;
            if (msCollected < MS_SAMPLES) msCollected++;
        }
        lastTickTime = now;
        ...
        if (msCollected < MS_SAMPLES) return;
