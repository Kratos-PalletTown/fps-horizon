...
**FPS Horizon** is a client-side Fabric mod for Minecraft 1.21.1 that dynamically adjusts render distance and simulation distance based on your average FPS and server tick time, keeping the game smooth without any manual tweaking.
...
| Cooldown after decreasing | 30 ticks | Wait time after reducing RD (20 ticks = 1 second) |
...
| Cooldown after decreasing | 30 ticks | Wait time after reducing SD |
...
### 📋 Changelog
> ℹ️ Version history below carries over from the original mod, now built for Fabric 1.21.1.
...
- Full synchronization with Sodium — the mod now updates the internal RenderSectionManager and calls markGraphDirty() whenever render distance changes.
...
- Integration with Sodium via Mixins

---

## 🇦🇷 Español

**FPS Horizon** es un mod cliente de Fabric para Minecraft 1.21.1 que ajusta automáticamente la distancia de renderizado y la distancia de simulación según el promedio de FPS y el tiempo de tick del servidor, manteniendo el juego fluido sin configuración manual.
...
1. Instalá [Fabric Loader](https://fabricmc.net/) para Minecraft 1.21.1
...
### 📋 Changelog
> ℹ️ El historial de versiones a continuación proviene del mod original, ahora construido para Fabric 1.21.1.
...
- Sincronización total con Sodium — el mod ahora actualiza el RenderSectionManager y llama a markGraphDirty() cada vez que cambia la distancia de renderizado.
...
- Integración con Sodium vía Mixins
